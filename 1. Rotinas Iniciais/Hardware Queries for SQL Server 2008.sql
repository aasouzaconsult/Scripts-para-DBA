-- Hardware Diagnostic Queries for SQL Server 2008
...
-- Hardware information from SQL Server 2008 and 2008 R2  (Query 3)
...
-- Good basic information about OS memory amounts and state  (Query 13)
...
-- SQL Server Process Address space info  (Query 14) 
...
-- Page Life Expectancy (PLE) value for each NUMA node in current instance  (Query 15)
...
-- Memory Clerk Usage for instance  (Query 16)
...
-- Missing Indexes for all databases by Index Advantage  (Query 17)
...
-- Top Cached SPs By Execution Count (SQL 2008)  (Query 19)
...
-- Breaks down buffers used by current database by object (table, index) in the buffer cache  (Query 20)
...
-- Get Table names, row counts, and compression status for clustered index or heap  (Query 21)
...
--- Index Read/Write stats (all tables in current DB) ordered by Reads  (Query 22)
...
--- Index Read/Write stats (all tables in current DB) ordered by Writes  (Query 23)
SELECT OBJECT_NAME(s.[object_id]) AS [ObjectName], i.name AS [IndexName], i.index_id,
...
-- Get the average full backup size by month for the current database (Query 24)
