...
* e função são criadas na base de dados ADVENTUREWORKS. No entanto, as mesmas podem ser criadas			*
...
********************************************************************************************************/
-- Se pretende criar as objetos em uma banco diferente de "AdventureWorks", substitua este pelo nome 
...
--		EXEC AdventureWorks..usp_docservidor 'C:\temp\doc_servidor' 
-- Executada documentação apenas dos logins
--		EXEC AdventureWorks..usp_doclogins 'C:\temp\doc_servidor' 
-- Executada documentação apenas dos usuários existentes nos DBs
--		EXEC AdventureWorks..usp_docusers 'C:\temp\doc_servidor' 
...
* 																*
* Criada por default no AdventureWorks							*
* Ex.: exec AdventureWorks..usp_doclogins 'C:\temp\doc_servidor'*
****************************************************************/
...
* 																*
* Criada por default no AdventureWorks							*
* Ex.: exec AdventureWorks..usp_docusers 'C:\temp\doc_servidor'	*
****************************************************************/
...
* 																*
* Criada por default no AdventureWorks							*
* Ex.: exec AdventureWorks..usp_docdbs 'C:\temp\doc_servidor'	*
****************************************************************/
...
* 								*
* Criada por default no AdventureWorks					*
* Ex.: exec AdventureWorks..usp_docdevices 'C:\temp\doc_servidor'		*
****************************************************************/
...
* 																*
* Criada por default no AdventureWorks							*
* Ex.: exec AdventureWorks..usp_docjobs 'C:\temp\doc_servidor'	*
****************************************************************/
...
* 																*
* Criada por default no AdventureWorks							*
* Ex.: exec AdventureWorks..usp_doclinkedsrv 'C:\temp\doc_servidor'	*
****************************************************************/
...
* 								*
* Criada por default no AdventureWorks					*
* Ex.: exec AdventureWorks..usp_docconfig 'C:\temp\doc_servidor'		*
****************************************************************/
...
*             															*
* Criada por default no AdventureWorks									*
* Ex.: AdventureWorks..usp_docservidor 'C:\temp\doc_servidor'           *
************************************************************************/

-- ATENÇÃO !!
-- ESTA É A PROCEDURE PRINCIPAL
-- Se estiver criando as procedures em uma base <> da AdventureWorks, altere o nome da base também nas linhas abaixo.
...
exec @lgn = AdventureWorks.dbo.usp_doclogins @Caminho

print '3. Executando Documentacao dos *** Usuários ***'
exec @usr = AdventureWorks.dbo.usp_docusers @Caminho

print '4. Executando Documentacao dos *** Banco de Dados ***'
exec @dbs = AdventureWorks.dbo.usp_docdbs @Caminho

print '5. Executando Documentacao dos *** Devices de Backup ***'
exec @dev = AdventureWorks.dbo.usp_docdevices @Caminho

print '6. Executando Documentacao dos *** Jobs ***'
exec @job = AdventureWorks.dbo.usp_docjobs @Caminho

print '7. Executando Documentacao dos *** Linked Servers ***'
exec @lnk = AdventureWorks.dbo.usp_doclinkedsrv @Caminho

print '8. Executando Documentacao da *** sp_configure ***'
exec @spconf = AdventureWorks.dbo.usp_docconfig @Caminho
...
BEGIN 
  ...
  -- Add the job steps
  EXECUTE @ReturnCode = msdb.dbo.sp_add_jobstep @job_id = @JobID, @step_id = 1, @step_name = N'Documenta_Servidor', @command = N'EXEC AdventureWorks..usp_docservidor ''C:\temp\doc_servidor''', @database_name = N'master', @server = N'', @database_user_name = N'', @subsystem = N'TSQL', @cmdexec_success_code = 0, @flags = 0, @retry_attempts = 0, @retry_interval = 1, @output_file_name = N'C:\temp\doc_servidor\Documenta_Servidor.log', @on_success_step_id = 0, @on_success_action = 1, @on_fail_step_id = 0, @on_fail_action = 2
  IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback 
